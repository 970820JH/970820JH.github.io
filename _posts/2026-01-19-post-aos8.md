...
앱 실행 시점에 서명키를 확인하는 코드를 구현함으로써, 앱 위변조 검증
...
int numBytes;                           
PackageInfo package Info = packageInfo = this.getPackafeManager().              
...
FileInptStream is = new FileInputStream(appPath);           // APK파일을 읽기 위한 입력 스트림 생성
