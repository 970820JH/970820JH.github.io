...
<br>
ㄴ서비스 : 화면없는 백그라운드 작업 컴포넌트(내부 백엔드 로직)<br>
ㄴ노출된 컴포넌트(서비스 & 액티비티) : 서비스와 액티비티에서 android:enabled 속성은 서비스가 사용 가능한지를 지정하며, android:exported 속성은 외부 앱이 서비스에 접근할 수 있는지를 지정<br>
ㄴ인텐트 필터 : 어떤 Intent를 받을지 정하는 조건표, 컴포넌트에 대한 인테트를 정의 → action 과 category 두 요소의 조합에 따라 컴포넌트의 외부 노출 범위가 결정됨 <br>
 cf. 인텐트는 개발딴에서 컴포넌트를 실핼하기 위한 메시지 요청서<br>

<img src="/assets/images/icon/action1.png" style="max-width:100%; border:1px solid #000; padding:4px;">
